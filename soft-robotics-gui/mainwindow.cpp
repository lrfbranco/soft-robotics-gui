  ...
  mTag2(nullptr)
{
  ...
  QCPAxis * a = mPlot->axisRect()->addAxis(QCPAxis::atRight);
  ...
  a->setRange(-1.3, 1.3);
  mPlot->yAxis2->setRange(-1.3, 1.3);

  //connect(mPlot->yAxis2, SIGNAL(rangeChanged(QCPRange)), secondRightAxis, SLOT(setRange(QCPRange)));
//  connect(secondRightAxis, SIGNAL(rangeChanged(QCPRange)), mPlot->yAxis2, SLOT(setRange(QCPRange)));
